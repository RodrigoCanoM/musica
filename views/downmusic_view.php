...
<body>
<?php
//Obtemos la ruta de controllers para realizar el pagook y pagoko
	$host= $_SERVER["HTTP_HOST"];
	$url= $_SERVER["REQUEST_URI"];
	$rutaController= "http://" . $host . $url;
	
	$aqui= strripos($rutaController,"downmusic.php");
	$rutaController=substr($rutaController,0,$aqui);
	echo $rutaController;
...
if ($_SERVER["REQUEST_METHOD"] == "POST" ){
	if(isset($_POST['compraFin'])){
		if (count($_SESSION['cesta'])>0) {
			...
			$trans = "0";
			$url = "";
			$urlOKKO = $rutaController."controllers/pagook.php";
			$urlKOOK = $rutaController."controllers/pagoko.php";
			$id = time();
